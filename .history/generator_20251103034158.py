# generate_only.py
# Use saved weights to generate a new ABC tune (no training).
...
# --- Model (must match the one used during training) ---
...
def build_vocab_and_maps(songs):
    joined = "\n\n".join(songs)
    vocab = sorted(set(joined))         # must match training (same sort)
...
def main():
    ...
    # Load the SAME dataset to rebuild the SAME vocab order used in training
    # (This matches your training script behavior.)
    try:
